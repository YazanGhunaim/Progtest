...
int centerSymmetry(int matrix[MAX][MAX], int rows);
...
int RowsColumns(int array[store + 1], int count[MAX], int *rows, int *columns);
...
int main()
{
    ...
    // RowsColumns(storage, elementCount, &rows, &columns);
    ...
    printf("Matrix:\n");
    if (RowsColumns(storage, elementCount, &rows, &columns) || rows < 0 || rows > MAX || columns > MAX)
    {
        printf(invalid);
        return EXIT_FAILURE;
    }
    for (int i = 0; i < rows; i++)
    {
        ...
        {
            if (elementCount[i] != elementCount[j])
            {
                printf(invalid);
                return 1;
            }
...
int centerSymmetry(int matrix[MAX][MAX], int rows)
{
    ...
    for (int i = 0; i < middleRow; i++)
    {
        ...
        {
            if (matrix[i][j] != matrix[rows - i - 1][rows - j - 1])
                return 0;
        }
    }
